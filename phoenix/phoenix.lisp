...
(defun start-time-of-day-cron ()
  (let ((timer-params '((:morning 7) (:afternoon 12) (:evening 16)))
        (utc-offset 4))
    (loop for timer-param in timer-params do
      (local-time:with-decoded-timestamp (:day day :month month :year year)
                                         (local-time:today)
        (sb-ext:schedule-timer (sb-ext:make-timer (lambda () (time-of-day-tasks (car timer-param))))
                               (local-time:timestamp-to-universal
                                (local-time:encode-timestamp
                                 0 0 0 (+ (cadr timer-param) utc-offset) day month year))
